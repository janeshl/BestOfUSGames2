...
function render(){
  ...
  const status=waiting
    ? `🔨 Auctioneer: ${close>0?`waiting ${close}s for a higher bid…`:'closing the bid…'}`
    : '🎙️ Auctioneer: Bidding is open. Bid, or choose No Interest.';
  app.innerHTML=`
    <div class="auction-rules card"><div><b>20-player auction</b> · 4 pools · No fixed timer</div><div class="auction-pool-list">${poolSummary}</div><div><b>Squad:</b> 5–6 buys · Must include 🧤 1 WK, 🏏 1 batsman, 🎯 1 bowler</div></div>
    <div class="auction-teams-top">${teams}</div>
    <div class="auction-pool-banner">POOL: ${roleIcon(state.poolName)} ${esc(state.poolName)} <span>Player ${state.index+1}/20</span></div>
    <div class="auction-main compact-auction">
      <div class="auction-player-card card">
        ...
        <div class="auction-actions">
          <button class="btn" ${bidBusy||waiting&&leader==='Your Team'?'disabled':''} onclick="bid()"><span>💰 Bid + ₹0.5 Cr</span></button>
          <button class="btn auction-skip-btn" ${waiting||bidBusy?'disabled':''} onclick="skipPlayer()"><span>🚫 No Interest / Skip</span></button>
        </div>
        ${state.auctionClosed?`<div class="auction-closed">🔨 ${state.currentBidder?'SOLD':'UNSOLD'} — Auctioneer has closed the bid.</div><button class="btn" onclick="nextPlayer()"><span>${state.index+1>=state.total?'Finish Auction':'Next Player →'}</span></button>`:''}
      </div>
